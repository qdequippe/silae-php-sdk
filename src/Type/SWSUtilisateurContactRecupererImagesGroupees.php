<?php

namespace Qdequippe\SilaePhpSdk\Type;

use Phpro\SoapClient\Type\RequestInterface;

class SWSUtilisateurContactRecupererImagesGroupees implements RequestInterface
{
    /**
     * @var string
     */
    private $Token;

    /**
     * @var int
     */
    private $ID_DOMAINE;

    /**
     * @var int
     */
    private $ID_CLIENT;

    /**
     * @var int
     */
    private $ID_DROIT;

    /**
     * @var int
     */
    private $NatureImage;

    /**
     * @var \Qdequippe\SilaePhpSdk\Type\ArrayOfInt
     */
    private $ID_IMAGES;

    /**
     * Constructor
     *
     * @var string $Token
     * @var int $ID_DOMAINE
     * @var int $ID_CLIENT
     * @var int $ID_DROIT
     * @var int $NatureImage
     * @var \Qdequippe\SilaePhpSdk\Type\ArrayOfInt $ID_IMAGES
     */
    public function __construct($Token, $ID_DOMAINE, $ID_CLIENT, $ID_DROIT, $NatureImage, $ID_IMAGES)
    {
        $this->Token = $Token;
        $this->ID_DOMAINE = $ID_DOMAINE;
        $this->ID_CLIENT = $ID_CLIENT;
        $this->ID_DROIT = $ID_DROIT;
        $this->NatureImage = $NatureImage;
        $this->ID_IMAGES = $ID_IMAGES;
    }

    /**
     * @return string
     */
    public function getToken()
    {
        return $this->Token;
    }

    /**
     * @param string $Token
     * @return SWSUtilisateurContactRecupererImagesGroupees
     */
    public function withToken($Token)
    {
        $new = clone $this;
        $new->Token = $Token;

        return $new;
    }

    /**
     * @return int
     */
    public function getID_DOMAINE()
    {
        return $this->ID_DOMAINE;
    }

    /**
     * @param int $ID_DOMAINE
     * @return SWSUtilisateurContactRecupererImagesGroupees
     */
    public function withID_DOMAINE($ID_DOMAINE)
    {
        $new = clone $this;
        $new->ID_DOMAINE = $ID_DOMAINE;

        return $new;
    }

    /**
     * @return int
     */
    public function getID_CLIENT()
    {
        return $this->ID_CLIENT;
    }

    /**
     * @param int $ID_CLIENT
     * @return SWSUtilisateurContactRecupererImagesGroupees
     */
    public function withID_CLIENT($ID_CLIENT)
    {
        $new = clone $this;
        $new->ID_CLIENT = $ID_CLIENT;

        return $new;
    }

    /**
     * @return int
     */
    public function getID_DROIT()
    {
        return $this->ID_DROIT;
    }

    /**
     * @param int $ID_DROIT
     * @return SWSUtilisateurContactRecupererImagesGroupees
     */
    public function withID_DROIT($ID_DROIT)
    {
        $new = clone $this;
        $new->ID_DROIT = $ID_DROIT;

        return $new;
    }

    /**
     * @return int
     */
    public function getNatureImage()
    {
        return $this->NatureImage;
    }

    /**
     * @param int $NatureImage
     * @return SWSUtilisateurContactRecupererImagesGroupees
     */
    public function withNatureImage($NatureImage)
    {
        $new = clone $this;
        $new->NatureImage = $NatureImage;

        return $new;
    }

    /**
     * @return \Qdequippe\SilaePhpSdk\Type\ArrayOfInt
     */
    public function getID_IMAGES()
    {
        return $this->ID_IMAGES;
    }

    /**
     * @param \Qdequippe\SilaePhpSdk\Type\ArrayOfInt $ID_IMAGES
     * @return SWSUtilisateurContactRecupererImagesGroupees
     */
    public function withID_IMAGES($ID_IMAGES)
    {
        $new = clone $this;
        $new->ID_IMAGES = $ID_IMAGES;

        return $new;
    }
}

